/**
 * 卡片注册表 —— 原则三「能力可枚举」的落点，**网页版/桌面客户端与 H5/APP 共用这一份**。
 ...
 *
 * 🆕 2026-09-24 从 src/h5/ 搬到 src/shared/：网页版原来**一张卡都没有**
 * （连 cards 字段都不认），用户在电脑上看不到「通过/驳回/确认发出」。
 * 搬过来而不是抄一份 —— 这张表里是**端点、method、字段名**，抄两份意味着
 * 哪天后端改了路径，有一端会静默打到不存在的地址上。
 *
 * ⚠️ 两个前端的 axios 实例不是同一个（H5 那个刻意不引 element-plus），
 *    所以 http 由各自在启动时注入，见 setCardHttp。
 */

/** 只用到这三个方法，不要求传完整的 axios 实例 */
export interface CardHttp {
  get: (url: string, config?: any) => Promise<any>
  post: (url: string, body?: any, config?: any) => Promise<any>
  put: (url: string, body?: any, config?: any) => Promise<any>
}

let _http: CardHttp | null = null

/** 各自的入口在启动时调一次。H5 传 h5/http 的实例，网页版传 api/index 的实例。 */
export function setCardHttp(h: CardHttp): void {
  _http = h
}

/** 取注入的 http。没注入就**明确报错**，别让它以 undefined 往下跑出一堆看不懂的栈。 */
function http(): CardHttp {
  if (!_http) throw new Error('卡片注册表还没注入 http（启动时要调 setCardHttp）')
  return _http
}
