...
- **第 13 批反馈 #267-#271（未提交，待授权发版）**：#271 供应商账目按分类聚排；#270 钣金装配表工艺1/工艺2列改 autocomplete（本列历史值可再选，对齐 SheetmetalGrid 模式）；#269 设计部新增「冷作图纸」(kind=coldwork_pkg) 上传推送钣金组+钣金组 tab 加图纸列；#268 User.grant_menus 按账号开通管理组菜单（用户管理弹窗勾选「字典设置」，接口 PUT /admin/users/{uid}/grant-menus；material-dict 写接口仍全员开放，gap 已知未收）；#267 封板组「已推送 N」旁打包下载（复用 POST /attachments/package）。测试：`test_produce_groups.py`（含 coldwork 断言）、`test_dict_menu_grant.py` PASSED；vue-tsc exit=0
- 最近三期交付：
