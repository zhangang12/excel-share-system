...
- **一次取数供多个 tab 用时，服务端不能按 tab 所依据的同一个字段收窄**（2026-08-01 #334/#335 教训）：
  设计部工作台一次拉 `orders` 供「我的订单(待接单/进行中/已完成)」三个 tab 客户端分流，
  而顶部状态下拉默认「进行中」被传成服务端 `proj_status` → 后端 `DeptOrder.status != done` 把已完成的
  在服务端删光 → 客户端 `myDone` **恒为空且不报错**，陈立新 29 条、赵仁辉 5 条已完成单直接消失。
  **改任何筛选口径前，先查清这份数据还被谁消费**（`grep` 那个 ref/computed 的所有引用）；
  同一维度不要放两个控件（下拉 + tab 抢 status 就是本次的设计错误）。
