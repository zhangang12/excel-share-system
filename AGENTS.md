...
- **桌面客户端已上线**：`5d585cf` 一期、`9a380ed` 白屏修复+品牌美化（vite base './' + hash 路由）、`753001f` release.sh 修复；更新通道 1.0.1 在跑。运维口径见「关键约定」桌面客户端条目
- **第 13 批反馈 #267-#271 已上线**（`e919ec4`，客户端 1.0.2 同步）：#271 账目分类聚排、#270 工艺列 autocomplete、#269 冷作图纸（coldwork_pkg）推钣金组+钣金组图纸列、#268 字典菜单按人授权、#267 图纸打包下载
- **一级菜单按账号配置（未提交，待授权发版）**：ROLE_MENUS 运行时废除 → `User.menus` 按账号清单（`PUT /admin/users/{uid}/menus` + 用户管理「菜单权限」弹窗，含业务+管理组两组勾选）；ROLE_DEFAULT_MENUS 仅作建号预填/backfill 模板；存量启动时 `backfill_user_menus` 零感知迁移（角色并集∪grant_menus∪messages/oa）；`grant-menus` 接口保留为旧客户端包装；MainLayout 的 dict-admin/管理组硬编码渲染 gap 已修为 menus 驱动。测试：新 `test_account_menus.py` + 全量 43 个跑完，基线 16 个修好 mr_probe（m01 减半），清单外零新增失败；vue-tsc exit=0
- 最近三期交付：
