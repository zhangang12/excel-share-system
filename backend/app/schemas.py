...
class SupplierIn(BaseModel):
    ...
    settle_type: Optional[str] = None      # 现金/月结/无账期
    settle_days: Optional[int] = None      # 账期天数
...
class SupplierRow(BaseModel):
    ...
    status: str = "active"
    # 账目一览口径（按当前用户可见范围聚合）
    recv_total: float = 0     # 送货单金额(采购合计)
    invoiced: float = 0       # 已开票
    to_invoice: float = 0     # 待开票
    paid: float = 0           # 已付款
    owed: float = 0           # 欠款
    item_count: int = 0
...
class PurchaseItemRow(BaseModel):
    ...
    to_invoice: float = 0      # 待开票 = 收货金额 − 开票金额
    pay_date: Optional[str] = None
    pay_amount: float = 0
    owed: float = 0            # 欠款 = 收货金额 − 付款金额
    buyer_uid: Optional[int] = None
...
class PurchaseItemListOut(BaseModel):
    rows: list[PurchaseItemRow]
    total: int = 0
    # 合计行（当前筛选范围）
    recv_total: float = 0
    invoiced: float = 0
...
class BatchSettleIn(BaseModel):
    ids: list[int] = []
    date: Optional[str] = None             # 开票/付款日期（YYYY-MM-DD）


class TrendBucket(BaseModel):
    key: str                  # 2026-06 / 2026-Q2 / 2026
    recv: float = 0
    invoiced: float = 0
    paid: float = 0
    owed: float = 0
...
class ProcureSummary(BaseModel):
    month_total: float = 0
    quarter_total: float = 0
    year_total: float = 0
    recv_total: float = 0
    owed_total: float = 0
    monthly: list[TrendBucket] = []        # 当年各月趋势
    by_buyer: list[BuyerStat] = []
