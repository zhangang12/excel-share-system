...
class Supplier(Base):
    """🆕 采购模块 · 供应商档案。结算方式/账期是关键属性，决定欠款到期提醒。
    停用而非删除（有采购明细的不可物理删，保数据可追溯）。"""
    __tablename__ = "suppliers"

    id: Mapped[int] = mapped_column(primary_key=True)
    name: Mapped[str] = mapped_column(String(128), index=True)          # 公司名称
    category: Mapped[Optional[str]] = mapped_column(String(32), index=True)  # 分类：外协加工/标准件/不锈钢原料/激光件/电气件/运输/其他
    contact: Mapped[Optional[str]] = mapped_column(String(64))          # 联系人
    phone: Mapped[Optional[str]] = mapped_column(String(64))
    address: Mapped[Optional[str]] = mapped_column(String(255))
    tax_no: Mapped[Optional[str]] = mapped_column(String(64))           # 税号
    bank_name: Mapped[Optional[str]] = mapped_column(String(128))       # 开户行
    bank_account: Mapped[Optional[str]] = mapped_column(String(64))     # 银行账号
    settle_type: Mapped[str] = mapped_column(String(16), default="月结")  # 现金/月结/无账期
    settle_days: Mapped[Optional[int]] = mapped_column()               # 账期天数（月结时有效）
...
class PurchaseItem(Base):
    """🆕 采购模块 · 采购明细（=年度采购明细表，唯一录入入口）。
    供应商账目一览 / 每家供应商报表 / 月季年汇总 全部由此自动派生。
    欠款 = 收货金额 − 付款金额；待开票 = 收货金额 − 开票金额。
    行级隔离：采购员仅见 buyer_uid=本人 的明细；admin/manager 全量。"""
    __tablename__ = "purchase_items"

    id: Mapped[int] = mapped_column(primary_key=True)
    recon_status: Mapped[str] = mapped_column(String(16), default="待对账", index=True)  # 待对账/已对账
    delivery_date: Mapped[Optional[str]] = mapped_column(String(10), index=True)  # 送货时间 YYYY-MM-DD
    supplier_id: Mapped[int] = mapped_column(ForeignKey("suppliers.id"), index=True)
    supplier_name: Mapped[Optional[str]] = mapped_column(String(128))   # 快照(展示/导入)
    contract_no: Mapped[Optional[str]] = mapped_column(String(64))      # 合同编号
    project_no: Mapped[Optional[str]] = mapped_column(String(64), index=True)  # 项目编号(对齐现有项目)
    delivery_no: Mapped[Optional[str]] = mapped_column(String(64))      # 送货单号
    item_name: Mapped[Optional[str]] = mapped_column(String(255))       # 名称
    spec: Mapped[Optional[str]] = mapped_column(String(255))            # 规格型号
    qty: Mapped[Optional[float]] = mapped_column()                      # 数量
    unit_price: Mapped[Optional[float]] = mapped_column()               # 单价
    recv_amount: Mapped[float] = mapped_column(default=0.0)             # 收货金额
    invoice_date: Mapped[Optional[str]] = mapped_column(String(10))     # 开票日期
    tax_rate: Mapped[Optional[str]] = mapped_column(String(16))         # 税率
    invoice_amount: Mapped[float] = mapped_column(default=0.0)          # 开票金额
    pay_date: Mapped[Optional[str]] = mapped_column(String(10))         # 付款日期
    pay_amount: Mapped[float] = mapped_column(default=0.0)              # 付款金额
    buyer_uid: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"), index=True)  # 采购负责人
    note: Mapped[Optional[str]] = mapped_column(Text)
    created_by: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), index=True)
    ...
    supplier: Mapped[Optional["Supplier"]] = relationship(lazy="joined")
    buyer: Mapped[Optional["User"]] = relationship(lazy="joined", foreign_keys=[buyer_uid])
